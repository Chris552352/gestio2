...
// 1. Données des enseignants camerounais
$enseignants = [
    [
        'nom' => 'Nze',
        'prenom' => 'Jean',
...
// 2. Données des étudiants camerounais
$etudiants = [
    [
        'nom' => 'Nkeng',
        'prenom' => 'Pierre',
...
// 3. Fonction pour générer un mot de passe aléatoire
...
// 4. Fonction pour hacher un mot de passe
...
foreach ($enseignants as $enseignant) {
    ...
    $sql = "INSERT INTO utilisateurs (nom, prenom, email, password, role) VALUES (?, ?, ?, ?, 'enseignant')";
...
foreach ($etudiants as $etudiant) {
    ...
    // Insérer l'étudiant
    $sql = "INSERT INTO utilisateurs (nom, prenom, email, password, role, telephone) VALUES (?, ?, ?, ?, 'etudiant', ?)";
    if (db_exec($sql, [$etudiant['nom'], $etudiant['prenom'], $etudiant['email'], $hashed_password, $etudiant['telephone']])) {
        ...
        // Inscrire l'étudiant dans les cours (dans chaque cours avec une probabilité de 70%)
        $cours = db_query("SELECT id FROM cours");
        foreach ($cours as $c) {
            if (rand(1, 100) <= 70) { // 70% de chance d'être inscrit
                $sql = "INSERT IGNORE INTO inscriptions (etudiant_id, cours_id) VALUES (?, ?)";
                if (db_exec($sql, [$etudiant_id, $c['id']])) {
                    show_message("Étudiant inscrit au cours " . htmlspecialchars($c['nom']) . " avec succès.", 'success');
                }
            }
        }
    } else {
